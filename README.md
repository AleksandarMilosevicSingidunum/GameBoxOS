...
Latest verified increments: RetroArch Android handoff now supports launcher ROM extras, package aliases, actionable controller guidance, and an ACTION_VIEW compatibility fallback; catalog refresh now distinguishes normal success, offline bundled fallback, remote-provider fallback, and terminal error; Store recovery messages are accessible and actionable; optional TheGamesDB transport or per-entry enrichment failures no longer block the authorized catalog; TheGamesDB discovery now resolves provider box art, fan art, clear logos, and screenshots from the documented `include=boxart` payload and offers fixed PS2/GameCube/Wii/PSP/Dreamcast/3DS/Switch/Homebrew filters with an up-to-20-title-per-console sync; cloud-save and WebDAV/S3 catalog transports use bounded retries and strict endpoint/size validation; save backups are size-bounded and restore paths are game-scoped; emulator handoffs reject malformed content URIs; diagnostics redact bearer and credential-shaped tokens; signed release manifests require rollback tags; Android and Windows CI passed on each merged increment.
