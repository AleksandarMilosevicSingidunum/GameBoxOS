...
`MainActivity` as an Android `HOME` / `DEFAULT` candidate. GameBox does not silently
replace the user's launcher.
...
candidate. That test proves registration only; it does not prove Honor firmware boot,
unlock or external-display behavior.
